...
from src.path import create_path
...
def main():
    ...
    latestblock = 0
    for _ in range(10):
        print(f'Latest known block is {latestblock}')
        newblock.get_latestBlock()
        currentblock = newblock.currentblock
        if currentblock > latestblock:
            print('There is a new block available')
            newblock.process_block_metadata()
            newblock.export_block('csv', 's3')
            latestblock = currentblock
        else:
            print('No new blocks have been mined')
